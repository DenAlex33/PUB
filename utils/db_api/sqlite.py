...
def add_userxref(user_id, user_login, user_name, balance, all_refill, reg_date,aa,arg):
    with sqlite3.connect(path_to_db) as db:
        aa = 0
        db.execute("INSERT INTO storage_users "
                   "(user_id, user_login, user_name, balance, all_refill, reg_date, refbal,ref,refkol) "
                   "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                   [user_id, user_login, user_name, balance, all_refill, reg_date, aa, arg, aa])
        db.commit()
...
def add_userx(user_id, user_login, user_name, balance, all_refill, reg_date):
    with sqlite3.connect(path_to_db) as db:
        ...
        db.execute("INSERT INTO storage_users "
                   "(user_id, user_login, user_name, balance, all_refill, reg_date, refbal,ref,refkol) "
                   "VALUES (?, ?, ?, ?, ?, ?, ?, ?,?)",
                   [user_id, user_login, user_name, balance, all_refill, reg_date, 0, 0,0])
        db.commit()
    ...
    with sqlite3.connect(path_to_db) as db:
        ...
        if len(check_create_users) == 10:
            ...
            db.execute("CREATE TABLE storage_users("
                       ...
                       "balance INTEGER, all_refill INTEGER, reg_date TIMESTAMP, refbal, ref, refkol)")
