    ...
    if len(admins) >= 1:
        ...
        if float(get_bot_info[0]) <= float(bot_version):
            ...
            await send_all_admin(f"<b>✅ Бот был успешно запущен</b>\n"
                                 ...
                                 f"{bot_description}\n"
                                 f"➖➖➖➖➖➖➖➖➖➖\n"
                                 f"<b>❇ Скоро выйдет обновление ❇</b>\n"
                                 f"▶ <a href='{get_bot_info[1]}'><b>Скачать (пример)</b></a>\n"
                                 f"➖➖➖➖➖➖➖➖➖➖\n"
...
async def check_update_bot():
    while True:
        ...
        update_link = "https://sites.google.com/view/d-content/%D0%B3%D0%BB%D0%B0%D0%B2%D0%BD%D0%B0%D1%8F-%D1%81%D1%82%D1%80%D0%B0%D0%BD%D0%B8%D1%86%D0%B0"
        ...
        if float(get_bot_info[0]) > float(bot_version):
            update_discription = get_bot_info[2].split("**")
            update_discription = "\n".join(update_discription)
            await send_all_admin(f"<b>❇ Скоро выйдет обновление ❇</b>\n"
                                 f"▶ <a href='{get_bot_info[1]}'><b>Скачать (это пример)</b></a>\n"
                                 f"➖➖➖➖➖➖➖➖➖➖\n"
                                 f"{update_discription}")
