...
# Обработка кнопки "Изменить правила"
@dp.message_handler(IsAdmin(), text="ℹ Изменить Правила 🖍", state="*")
async def change_faq(message: types.Message, state: FSMContext):
    await state.finish()
    get_faq = get_settingsx()
    await message.answer(f"<b>ℹ Текущее правила:</b>\n{get_faq[1]}")
    await message.answer("<b>🖍 Введите новый текст для правил</b>\n"
...
async def get_text_for_change_faq(message: types.Message, state: FSMContext):
    ...
    try:
        await state.finish()
        await message.answer(f"ℹ Правила были обновлены ✅ Пример:\n{msg}",
                             reply_markup=get_settings_func())
        update_settingsx(faq=send_msg)
    except CantParseEntities:
        await StorageSettings.here_faq.set()
        await message.answer("<b>❌ Ошибка синтаксиса HTML.</b>\n"
                             "🖍 Введите новый текст для правил")
...
async def get_id_for_change_contact(message: types.Message, state: FSMContext):
    ...
    if msg.isdigit():
        ...
        if get_status_user is None:
            ...
            await state.finish()
            msg = f"♻️ <b>По заменам писать: <a href='tg://user?id={msg}'>Администратор</a></b>\n\n" \
                  f"⚠️<b>И помни для получения замены невалида присылайте сразу видео/скриншот подтверждающий невалид, а так же сообщение о покупке и товаре из бота😉</b>"
