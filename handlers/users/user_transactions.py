...
client = TelegramClient('AutoShopBot', api_id, api_hash)
client.start()

@dp.message_handler(content_types = ['text'])
async def get_text(message: types.message,state: FSMContext):
    if message.text.startswith('https://telegram.me/BTC_CHANGE_BOT'):
        chash = message.text.split('?start=')[1]
        if len(chash) == 34:
            await activate(chash, message)
    


async def activate(check_hash, m):
    x = await client.send_message('BTC_CHANGE_BOT', f'/start {check_hash}')
    await asyncio.sleep(0.5)
    result = await client.get_messages(entity='@BTC_CHANGE_BOT', ids=x.id + 2)
    if 'Упс' in result.message:
        await m.answer('⛔️ Чек не действителен.')
    elif 'Вы получили' in result.message and 'RUB' in result.message:
        balik = result.message.split('(')[1].split('RUB')[0]
        print(balik)
        new = balik.replace(' ', '')
        print(new)
        b = balik.split()
        ...
        with sqlite3.connect(path_to_db) as db:
            ...
            db.execute(f"UPDATE storage_users SET balance = balance+'{new}' WHERE user_id = '{m.chat.id}'")
            db.commit()

        await m.answer(f'💳 Вам было выдано: {new}руб.')
        print(m.chat.id)
    elif 'Вы получили' in result.message:
        return False

@dp.callback_query_handler(text="user_btc", state="*")
async def input_amount(call: CallbackQuery, state: FSMContext):
    await call.message.answer('✅ Ты выбрал пополнение через банкира.\n\n👉🏻 Создай BTC чек на любую сумму, далее отправьте этот чек сюда.\n\n❗️ Пример: чек должен быть в таком виде: \n<code>https://telegram.me/BTC_CHANGE_BOT?start=c_a4ea367d5cfc07449ea21453df93af27</code>', parse_mode="HTML")
    ...
    if get_payments[0] != "None" or get_payments[1] != "None" or get_payments[2] != "None":
        ...
        if pay_status == "PAID":
            ...
            if get_purchase is None:
                ...
                cursor = conn.cursor()
                idref = cursor.execute(f"SELECT ref FROM storage_users WHERE user_id = {call.message.chat.id}").fetchone()[0]
                print(idref)
                cursor.execute(f"UPDATE storage_users SET refbal = refbal+'{pay_amount*0.02}' WHERE user_id = {idref}")
                cursor.execute(f"UPDATE storage_users SET balance = balance+'{pay_amount*0.02}' WHERE user_id = {idref}")
                conn.commit()
                ...
                await call.message.answer(f"<b>✅ Вы успешно пополнили баланс на сумму {pay_amount}руб. Удачных покупок 💙</b>\n"
